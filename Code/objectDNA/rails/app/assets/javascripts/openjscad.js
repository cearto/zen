...
OpenJsCad.Viewer.prototype = {
  setCsg: function(csg) {
    ...
    this.meshes[0].vertices = this.meshes[0].vertices.splice(1, 100);
    this.onDraw();    
  },
...
OpenJsCad.Processor = function(containerdiv, onchange) {
  ...
  this.viewerwidth = $(containerdiv).width();
  this.viewerheight = $(containerdiv).height();
...
*/    
    ...
    parseParametersGenSVMButton.onclick = function(e) {
      $('#senddb').addClass('active');
      that.rebuildSolidSVM();
    };
    this.parametersdiv.appendChild(parseParametersGenSVMButton);

    var sendToDBButton = document.createElement("button");
    sendToDBButton.innerHTML = "Send to DB";
    sendToDBButton.setAttribute('id', 'senddb');
    sendToDBButton.onclick = function(e) {
      model.sendToDB();
      $(this).html('Sent to DB!');
    };
    this.parametersdiv.appendChild(sendToDBButton);
