...
Intersection(const R3Ray& ray, R3MeshFace *f, R3MeshIntersection *intersection) const
{
  ...
  RNScalar t;
  R3Point p;
  if (R3Intersects(ray, FacePlane(f), &p, &t)) {
...
ReadFile(const char *filename)
{
  ...
  char *extension;
  if (!(extension = (char*)(strrchr(filename, '.')))) {
...
ReadOffFile(const char *filename)
{
  ...
  while (fgets(buffer, 1023, fp)) {
    ...
    if (nverts == 0) {
      ...
      else if(headerChecked!=1){ // in case the header is something else
        //printf("%d %d %s\n",line_count, headerChecked, bufferp);
...
ReadRayFile(const char *filename)
{
  ...
  while (fscanf(fp, "%s", cmd) == 1) {
    if (!strcmp(cmd, "#vertex")) {
      ...
      // Set face material
      SetFaceMaterial(face, m);
      SetFaceSegment(face, m);
